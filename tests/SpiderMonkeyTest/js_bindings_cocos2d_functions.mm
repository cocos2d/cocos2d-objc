...
JSBool JSPROXY_ccDrawSolidRect(JSContext *cx, uint32_t argc, jsval *vp) {
	...
	arg2 = *(ccColor4F*)JS_GetArrayBufferViewData( tmp_arg2, cx );
	if( ! ok ) return JS_FALSE;
...
JSBool JSPROXY_ccc4(JSContext *cx, uint32_t argc, jsval *vp) {
	...
	JSObject *typedArray = JS_NewUint8Array(cx, 4 );
	ccColor4B* buffer = (ccColor4B*)JS_GetArrayBufferViewData(typedArray, cx);
	*buffer = ret_val;
	JS_SET_RVAL(cx, vp, OBJECT_TO_JSVAL(typedArray));
...
JSBool JSPROXY_ccc4BFromccc4F(JSContext *cx, uint32_t argc, jsval *vp) {
	...
	arg0 = *(ccColor4F*)JS_GetArrayBufferViewData( tmp_arg0, cx );
	if( ! ok ) return JS_FALSE;
	ccColor4B ret_val;
...
JSBool JSPROXY_ccc4FEqual(JSContext *cx, uint32_t argc, jsval *vp) {
	...
	arg1 = *(ccColor4F*)JS_GetArrayBufferViewData( tmp_arg1, cx );
	if( ! ok ) return JS_FALSE;
	BOOL ret_val;
...
JSBool JSPROXY_ccc4FFromccc3B(JSContext *cx, uint32_t argc, jsval *vp) {
	...
	ccColor4F* buffer = (ccColor4F*)JS_GetArrayBufferViewData(typedArray, cx);
	*buffer = ret_val;
	JS_SET_RVAL(cx, vp, OBJECT_TO_JSVAL(typedArray));
