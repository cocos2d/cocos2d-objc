...
/**
 * Maps a CCFileUtil's device/resolution suffix string, for instance `CCFileUtilsSuffixiPadHD`, to a CocosBuilder resolution string, for instance `tablethd`.
 *
 * @param suffix A CCFileUtils resolution suffix, as defined by the CCFileUtilsSuffix* constants.
 * @returns A CocosBuilder resolution string, one of: `phone`, `phonehd`, `tablet`, `tablethd`. Returns `nil` if there is no matching CocosBuilder suffix for the given input suffix.
 * @since v3.3 and later
 */
+ (NSString *)ccFileUtilsSuffixToResolution:(NSString *)suffix;

/**
 * Returns the preferred CocosBuilder resolution string (`phone`, `phonehd`, `tablet`, `tablethd`) for the current Cocos2D setup by looking at the entries
