...
-(CCRenderBuffer)enqueueTriangles:(NSUInteger)triangleCount andVertexes:(NSUInteger)vertexCount withState:(CCRenderState *)renderState globalSortOrder:(NSInteger)globalSortOrder;
{
	...
	if(previous && previous->_renderState == renderState && previous->_globalSortOrder == globalSortOrder){
		...
		// Start a new command.
		#warning TODO GL_TRIANGLES, NSClassFromString(...)
		CCRenderCommandDraw *command = [[NSClassFromString(@"CCRenderCommandDrawGL") alloc] initWithMode:GL_TRIANGLES renderState:renderState first:firstElement count:elementCount globalSortOrder:globalSortOrder];
...
-(CCRenderBuffer)enqueueLines:(NSUInteger)lineCount andVertexes:(NSUInteger)vertexCount withState:(CCRenderState *)renderState globalSortOrder:(NSInteger)globalSortOrder;
{
	...
	CCRenderCommandDraw *command = [[NSClassFromString(@"CCRenderCommandDrawGL") alloc] initWithMode:GL_LINES renderState:renderState first:firstElement count:elementCount globalSortOrder:globalSortOrder];
	[_queue addObject:command];
	[command release];
