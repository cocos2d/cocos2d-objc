...
- (id)initWithEffects:(NSArray *)effects manglePrefix:(NSString *)prefix mangleExclusions:(NSSet *)exclusions stitchListIndex:(NSUInteger)stitchListIndex shaderStartIndex:(NSUInteger)shaderStartIndex;
{
    ...
    if((self = [super init]))
    {
        [self stitchEffects:effects manglePrefix:prefix mangleExclusions:exclusions stitchListIndex:stitchListIndex shaderStartIndex:shaderStartIndex];
    }
    return self;
}
...
- (void)stitchEffects:(NSArray *)effects manglePrefix:(NSString *)prefix mangleExclusions:(NSSet *)exclusions stitchListIndex:(NSUInteger)stitchListIndex shaderStartIndex:(NSUInteger)shaderStartIndex
{
    ...
    for (CCEffectImpl *effect in effects)
    {
        for (CCEffectShader *shader in effect.shaders)
        {
            ...
            CCEffectShaderBuilder *vtxBuilder = shader.vertexShaderBuilder;
            NSDictionary *prefixedVtxComponents = [CCEffectStitcher prefixComponentsFromBuilder:vtxBuilder withPrefix:shaderPrefix andExclusions:exclusions stitchListIndex:stitchListIndex];
            [CCEffectStitcher mergePrefixedComponents:prefixedVtxComponents fromShaderAtIndex:shaderIndex intoAllComponents:allVtxComponents];
            
            CCEffectShaderBuilder *fragBuilder = shader.fragmentShaderBuilder;
            NSDictionary *prefixedFragComponents = [CCEffectStitcher prefixComponentsFromBuilder:fragBuilder withPrefix:shaderPrefix andExclusions:exclusions stitchListIndex:stitchListIndex];
            [CCEffectStitcher mergePrefixedComponents:prefixedFragComponents fromShaderAtIndex:shaderIndex intoAllComponents:allFragComponents];
    ...
    CCEffectShaderBuilder *vtxBuilder = [[CCEffectShaderBuilder alloc] initWithType:CCEffectShaderBuilderVertex
    ...
    _shaders = @[[[CCEffectShader alloc] initWithVertexShaderBuilder:vtxBuilder fragmentShaderBuilder:fragBuilder]];
...
+ (NSDictionary *)prefixComponentsFromBuilder:(CCEffectShaderBuilder *)builder withPrefix:(NSString *)prefix andExclusions:(NSSet *)exclusions stitchListIndex:(NSUInteger)stitchListIndex
{
    NSMutableDictionary *prefixedComponents = [[NSMutableDictionary alloc] init];
    
    prefixedComponents[CCEffectStitcherUniforms] = [CCEffectStitcher uniformsByApplyingPrefix:prefix toUniforms:builder.uniforms withExclusions:exclusions];
    prefixedComponents[CCEffectStitcherVaryings] = [CCEffectStitcher varyingsByApplyingPrefix:prefix toVaryings:builder.varyings];
    prefixedComponents[CCEffectStitcherFunctions] = [CCEffectStitcher functionsByApplyingPrefix:prefix
    ...
    prefixedComponents[CCEffectStitcherTemporaries] = [CCEffectStitcher temporariesByApplyingPrefix:prefix toTemporaries:builder.temporaries stitchListIndex:stitchListIndex];
    prefixedComponents[CCEffectStitcherCalls] = [CCEffectStitcher callsByApplyingPrefix:prefix functionReplacements:prefixedComponents[CCEffectStitcherFunctions] toCalls:builder.calls];
...
+ (NSDictionary *)functionsByApplyingPrefix:(NSString *)prefix uniformReplacements:(NSDictionary *)uniformReplacements varyingReplacements:(NSDictionary *)varyingReplacements toFunctions:(NSArray *)functions
{
    ...
    {
        CCEffectFunction *prefixedFunction = [CCEffectStitcher effectFunctionByApplyingPrefix:prefix uniformReplacements:uniformReplacements varyingReplacements:varyingReplacements allFunctions:functions toEffectFunction:function];
...
{
    NSString *prefixedBody = [CCEffectStitcher functionBodyByApplyingPrefix:prefix uniformReplacements:uniformReplacements varyingReplacements:varyingReplacements allFunctions:(NSArray *)allFunctions toFunctionBody:function.body];
